# Command Center readiness — 2026-09-06
...
| Polish | Inspection/effects used old boxes, heartbeats restarted pulses, and connection text stayed misleading. | Persistent inspector with keyboard focus restoration, empty-floor persistence, stable CRT narration, surface-masked effects, bounded transients, physical station labels, and visible/accessibility connection updates. |
...
- One SQL statement owns ingest atomicity and deterministic ordering. Additive run metadata/visibility migration preserves immutable events, classifies known diagnostics, and rebuilds public latest state.
- `presentationModel.mjs` is the shared semantic layer for connection, task state, freshness, truthful narration, current-task count, and character labels; the DOM, CRT, scene, inspector, and history consume the same view.

## Verification evidence

- The full Node test suite passes, including clarity coverage for terminal durability, expired/disconnected activity, stable run grouping, event collapse/caps, diagnostic filtering, metadata validation, and the verified machine catalog.
...
- **12 disposable PostgreSQL checks passed:** migration/backfill preservation, diagnostic persistence/isolation, concurrent duplicate ingest, deterministic equal-timestamp ordering, older-event rejection, partial-state repair, rollback after injected failure, agent isolation, reload restoration, completed-job boundaries, run history, and cursor pagination without gaps or duplicates.
- Chromium browser coverage checks **rendered Phaser animations**, attendance, positions, persistent inspection targets, machine-directory keyboard activation/focus restoration, stable CRT transitions, history expansion/filter/pagination/new-work notice, loading/empty/unavailable/pagination-error states, phase-preserving effects, connection health, individual texture failures and reduced motion.
...
Browser screenshots and structured results are generated under `/tmp/cc-hardening-evidence`: `station-*.png`, `approach-*.png`, `viewport-*.png`, `page-*.png`, `fullscreen.png`, and `results.json`. Desktop and 390×844 full-page screenshots were visually reviewed for hierarchy, readable status/history copy, inspector stacking, ground placement, and occlusion. Reproduce them with `npm run command-center:verify:browser`; database checks use `npm run command-center:verify:db` with a disposable local `CC_TEST_DATABASE_URL`. Both runners reject non-local verification targets; browser fixtures never call ingest.
